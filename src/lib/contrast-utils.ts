...
export function getTextColorForBackground(bgHsl: string): string {
  ...
  if (!match) return '220 10% 15%'; // fallback to dark

  const [, h, s, l] = match.map(Number);
  const rgb = hslToRgb(h, s, l);
  const lum = getLuminance(...rgb);

  // Se o fundo é escuro (luminância < 0.5), retorna texto claro
  if (lum < 0.5) {
    return '220 15% 92%'; // off-white
  }

  // Se o fundo é claro, retorna texto escuro
  return '220 10% 15%'; // dark gray
}
...
export function autoFixContrast(element?: HTMLElement): void {
  ...
  // Pega cores do background e foreground
  ...
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      if (
        ...
        // Aguarda um frame para o CSS ser aplicado
